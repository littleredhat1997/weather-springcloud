package com.example.demo.task;

import com.alibaba.fastjson.JSON;
import com.example.demo.controller.WeatherController;
import com.example.demo.entity.City;
import com.example.demo.entity.WeatherResponse;
import com.example.demo.service.CityClient;
import com.example.demo.service.WeatherService;
import lombok.extern.slf4j.Slf4j;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.scheduling.annotation.Scheduled;
import org.springframework.stereotype.Component;

import java.util.List;

@Slf4j
@Component
public class DataTask {

    @Autowired
    private WeatherService weatherService;

    @Autowired
    private CityClient cityClient;

    @Scheduled(fixedRate = 1800000) // 1800s
    public void weatherDataSync() {
        try {
            List<City> cityList = cityClient.listCity();
            for (City city : cityList) {
                String cityName = city.getCityName();
                WeatherResponse weatherResponse = weatherService.getDataByCityName(cityName);
                WeatherController.weatherResponseMap.put(cityName, weatherResponse);
                log.info(cityName + ":" + JSON.toJSONString(weatherResponse));
            }
        } catch (Exception e) {
            log.error("解析失败：", e);
        }
    }
}
